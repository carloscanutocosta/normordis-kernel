...
use domain_telemetry::{
    AppUsageEvent, AppUsageStats, SessionId, TelemetryError, TelemetryRepository,
    UsageEventFilter, UsageEventId, UsageEventType, UsagePeriod,
};
...
impl TelemetryRepository for TelemetrySqliteStore {
    ...
    fn query_events(
        ...
        let event_type_str: Option<&str> =
            filter.event_type.as_ref().map(UsageEventType::as_str);
        let after_str: Option<String> =
            filter.occurred_after.map(|dt| dt.to_rfc3339());
        let before_str: Option<String> =
            filter.occurred_before.map(|dt| dt.to_rfc3339());
        ...
        let raw: Vec<(String, String, String, String, Option<String>, String, String, String)> =
            stmt.query_map(
                params![
    ...
    fn top_apps(
        ...
        Ok(raw
            .into_iter()
            .map(|(app_id, opened, closed, failed, sessions, users)| AppUsageStats {
                app_id,
                period:                    period.clone(),
                opened_count:              opened as u64,
                closed_count:              closed as u64,
...
mod tests {
    ...
    fn jan2025() -> UsagePeriod {
        UsagePeriod::Month { year: 2025, month: 1 }
    }
    ...
    fn record_and_query_event() {
        ...
        let e = event("ev-1", "app-x", "sess-1", "user-a", UsageEventType::AppOpened, ts(0));
        store.record(&e).unwrap();

        let result = store.query_events(&UsageEventFilter::default(), 10, 0).unwrap();
    ...
    fn record_duplicate_event_id_is_idempotent() {
        let (_dir, store) = open_tmp();
        let e = event("ev-dup", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(0));
        store.record(&e).unwrap();
        store.record(&e).unwrap(); // segunda vez — não deve errar

        let result = store.query_events(&UsageEventFilter::default(), 10, 0).unwrap();
        assert_eq!(result.len(), 1, "evento duplicado não deve ser inserido duas vezes");
    }

    #[test]
    fn query_filter_by_app_id() {
        let (_dir, store) = open_tmp();
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-y", "s2", "u1", UsageEventType::AppOpened, ts(1))).unwrap();

        let result = store
            .query_events(
                &UsageEventFilter { app_id: Some("app-x".into()), ..Default::default() },
                10, 0,
            )
            .unwrap();
        assert_eq!(result.len(), 1);
    ...
    fn query_filter_by_session_id() {
        let (_dir, store) = open_tmp();
        store.record(&event("e1", "app-x", "sess-a", "u1", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-x", "sess-a", "u1", UsageEventType::AppClosed, ts(100))).unwrap();
        store.record(&event("e3", "app-x", "sess-b", "u1", UsageEventType::AppOpened, ts(200))).unwrap();

        let result = store
            .query_events(
                &UsageEventFilter { session_id: Some("sess-a".into()), ..Default::default() },
                10, 0,
            )
            .unwrap();
    ...
    fn query_filter_by_event_type() {
        let (_dir, store) = open_tmp();
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-x", "s1", "u1", UsageEventType::AppFailed, ts(1))).unwrap();

        let result = store
            .query_events(
                &UsageEventFilter { event_type: Some(UsageEventType::AppFailed), ..Default::default() },
                10, 0,
            )
            .unwrap();
        assert_eq!(result.len(), 1);
    ...
    fn query_filter_by_user_id() {
        let (_dir, store) = open_tmp();
        store.record(&event("e1", "app-x", "s1", "alice", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-x", "s2", "bob",   UsageEventType::AppOpened, ts(1))).unwrap();

        let result = store
            .query_events(
                &UsageEventFilter { user_id: Some("alice".into()), ..Default::default() },
                10, 0,
            )
            .unwrap();
        assert_eq!(result.len(), 1);
    ...
    fn query_filter_by_date_range() {
        ...
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, base)).unwrap();
        store.record(&event("e2", "app-x", "s2", "u1", UsageEventType::AppOpened,
            base + Duration::days(10))).unwrap();

        let result = store
            .query_events(
                &UsageEventFilter { occurred_after: Some(base + Duration::days(5)), ..Default::default() },
                10, 0,
            )
            .unwrap();
        assert_eq!(result.len(), 1);
    ...
    fn query_limit_respected() {
        ...
        for i in 0..5 {
            store.record(&event(
                &format!("ev-{i}"), "app-x", &format!("s-{i}"), "u1",
                UsageEventType::AppOpened, ts(i * 100),
            )).unwrap();
        }
        assert_eq!(store.query_events(&UsageEventFilter::default(), 3, 0).unwrap().len(), 3);
    }
    ...
    fn query_pagination_with_offset() {
        ...
        let page1 = store.query_events(&UsageEventFilter::default(), 2, 0).unwrap();
        let page2 = store.query_events(&UsageEventFilter::default(), 2, 2).unwrap();
        let page3 = store.query_events(&UsageEventFilter::default(), 2, 4).unwrap();
        ...
        let all_ids: Vec<_> = page1.iter().chain(&page2).chain(&page3)
            .map(|e| e.event_id.as_str().to_owned()).collect();
        let unique: std::collections::HashSet<_> = all_ids.iter().collect();
        assert_eq!(all_ids.len(), unique.len(), "eventos duplicados entre páginas");
    }
    ...
    fn stats_basic_counts() {
        let (_dir, store) = open_tmp();
        store.record(&event("e1", "app-x", "s1", "alice", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-x", "s1", "alice", UsageEventType::AppClosed, ts(100))).unwrap();
        store.record(&event("e3", "app-x", "s2", "bob",   UsageEventType::AppOpened, ts(200))).unwrap();
        store.record(&event("e4", "app-x", "s2", "bob",   UsageEventType::AppFailed, ts(300))).unwrap();
    ...
    fn stats_avg_session_duration() {
        ...
        let t0 = ts(0);
        // Sessão 1: 1 segundo
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, t0)).unwrap();
        store.record(&event("e2", "app-x", "s1", "u1", UsageEventType::AppClosed,
            t0 + Duration::seconds(1))).unwrap();
        // Sessão 2: 3 segundos
        store.record(&event("e3", "app-x", "s2", "u1", UsageEventType::AppOpened, ts(10000))).unwrap();
        store.record(&event("e4", "app-x", "s2", "u1", UsageEventType::AppClosed,
            ts(10000) + Duration::seconds(3))).unwrap();

        let stats = store.stats_for_app("app-x", &jan2025()).unwrap().unwrap();
        let avg = stats.avg_session_duration_secs.unwrap();
        // Média de 1s e 3s = 2s
        assert!((avg - 2.0).abs() < 0.1, "avg_duration esperado ~2s, obtido {avg:.3}s");
    }
    ...
    fn stats_no_double_counting_with_multiple_close_events() {
        ...
        store.record(&event("e2", "app-x", "s1", "u1", UsageEventType::AppClosed,
            t0 + Duration::seconds(2))).unwrap();
        store.record(&event("e3", "app-x", "s1", "u1", UsageEventType::AppClosed,
            t0 + Duration::seconds(10))).unwrap();
    ...
    fn stats_out_of_period_ignored() {
        ...
        let feb = chrono::DateTime::parse_from_rfc3339("2025-02-01T00:00:00Z")
            .unwrap().with_timezone(&Utc);
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, feb)).unwrap();
    ...
    fn top_apps_sorted_by_opens() {
        let (_dir, store) = open_tmp();
        for i in 0..3 {
            store.record(&event(&format!("a{i}"), "app-a", &format!("sa{i}"), "u1",
                UsageEventType::AppOpened, ts(i * 100))).unwrap();
        }
        store.record(&event("b1", "app-b", "sb1", "u1", UsageEventType::AppOpened, ts(999))).unwrap();
        for i in 0..2 {
            store.record(&event(&format!("c{i}"), "app-c", &format!("sc{i}"), "u1",
                UsageEventType::AppOpened, ts(i * 100 + 5000))).unwrap();
        }
    ...
    fn top_apps_includes_session_count() {
        ...
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(0))).unwrap();
        store.record(&event("e2", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(100))).unwrap();
        store.record(&event("e3", "app-x", "s2", "u2", UsageEventType::AppOpened, ts(200))).unwrap();
    ...
    fn metadata_roundtrip() {
        ...
        let mut e = event("ev-m", "app-x", "s1", "u1", UsageEventType::ActionCompleted, ts(0));
        e.metadata = meta;
        store.record(&e).unwrap();

        let result = store.query_events(&UsageEventFilter::default(), 10, 0).unwrap();
        assert_eq!(result[0].metadata.get("action").map(String::as_str), Some("criar_documento"));
        assert_eq!(result[0].metadata.get("tipo_doc").map(String::as_str), Some("oficio"));
    }
    ...
    fn service_validates_empty_app_id() {
        ...
        let mut e = event("ev-1", "app-x", "s1", "u1", UsageEventType::AppOpened, ts(0));
        e.app_id = String::new();
        assert!(svc.record_event(e).unwrap_err().to_string().contains("app_id"));
    }
    ...
    fn top_apps_limit_respected() {
        ...
        for i in 0..5i64 {
            store.record(&event(
                &format!("ev-{i}"), &format!("app-{i}"), &format!("s-{i}"), "u1",
                UsageEventType::AppOpened, ts(i * 100),
            )).unwrap();
        }
    ...
    fn stats_day_period() {
        ...
        let day_ts = chrono::DateTime::parse_from_rfc3339("2025-01-15T09:00:00Z")
            .unwrap().with_timezone(&Utc);
        store.record(&event("e1", "app-x", "s1", "u1", UsageEventType::AppOpened, day_ts)).unwrap();

        let stats = store
            .stats_for_app("app-x", &UsagePeriod::Day(NaiveDate::from_ymd_opt(2025, 1, 15).unwrap()))
            .unwrap()
            .unwrap();
        assert_eq!(stats.opened_count, 1);

        assert!(store
            .stats_for_app("app-x", &UsagePeriod::Day(NaiveDate::from_ymd_opt(2025, 1, 16).unwrap()))
            .unwrap()
            .is_none());
    }
}
