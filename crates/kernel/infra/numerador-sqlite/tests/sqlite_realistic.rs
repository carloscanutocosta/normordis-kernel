...
use domain_numerador::{
    ActorRef, AssignedStatus, AssignmentFilter, AssignNumberRequest, ChangeStatusRequest,
...
fn oficio_seq(entity_id: &str, valid_from: NaiveDate, valid_to: Option<NaiveDate>) -> NumberingSequence {
    seq(
...
fn persistencia_entre_sessoes() {
    ...
    let s = oficio_seq("sf-setubal", NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), None);
    ...
    // Sessão 1: criar sequência e emitir 3 ofícios
    {
        ...
        db.assign(&req("doc-01", "sf-setubal", "oficio_at", "ccosta", Some("Pedido de certidão"), Some("João Silva")), dt(2026, 1, 10), "ref-01").unwrap();
        db.assign(&req("doc-02", "sf-setubal", "oficio_at", "ccosta", Some("Resposta ACSS"), Some("ACSS")), dt(2026, 1, 15), "ref-02").unwrap();
        db.assign(&req("doc-03", "sf-setubal", "oficio_at", "ccosta", Some("Pedido de informação"), Some("ATA")), dt(2026, 1, 20), "ref-03").unwrap();
    } // conexão fecha aqui

    // Sessão 2: reabrir e verificar que os dados persistiram
    {
        let db = open(&db_path);
        let all = db.list_assignments(&AssignmentFilter::default(), 100).unwrap();
        ...
        db.assign(&req("doc-04", "sf-setubal", "oficio_at", "mmatos", None, None), dt(2026, 2, 1), "ref-04").unwrap();
        db.assign(&req("doc-05", "sf-setubal", "oficio_at", "mmatos", None, None), dt(2026, 2, 5), "ref-05").unwrap();
    }
    ...
    // Sessão 3: verificar contador continuou do ponto correcto
    {
        ...
        let first = db.get_by_target(&NumberingKind::Document, "doc-01").unwrap().unwrap();
        assert_eq!(first.metadata.subject.as_deref(), Some("Pedido de certidão"));
...
fn isolamento_entre_entidades() {
    ...
    db.assign(&req("s-doc-1", "sf-setubal", "oficio_at", "u1", None, None), dt(2026, 3, 10), "ref-s1").unwrap();
    db.assign(&req("s-doc-2", "sf-setubal", "oficio_at", "u1", None, None), dt(2026, 3, 11), "ref-s2").unwrap();
    db.assign(&req("s-doc-3", "sf-setubal", "oficio_at", "u1", None, None), dt(2026, 3, 12), "ref-s3").unwrap();

    db.assign(&req("n-doc-1", "sf-sintra", "oficio_at", "u2", None, None), dt(2026, 3, 13), "ref-n1").unwrap();
    db.assign(&req("n-doc-2", "sf-sintra", "oficio_at", "u2", None, None), dt(2026, 3, 14), "ref-n2").unwrap();

    db.assign(&req("c-doc-1", "sf-cascais", "oficio_at", "u3", None, None), dt(2026, 3, 15), "ref-c1").unwrap();

    // Contadores são independentes por entidade
    let by_setubal = db.list_assignments(&AssignmentFilter {
    ...
    let by_sintra = db.list_assignments(&AssignmentFilter {
    ...
    let by_cascais = db.list_assignments(&AssignmentFilter {
...
fn reset_anual_do_contador() {
    ...
    let db = open(&db_path);
    db.upsert(&oficio_seq("sf-setubal", NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(), None)).unwrap();

    let mut db = open(&db_path);

    // 2025: emitir 5 ofícios
    for i in 1..=5u64 {
        let r = db.assign(&req(&format!("doc-2025-{i}"), "sf-setubal", "oficio_at", "u1", None, None), dt(2025, 6, i as u32), &format!("ref-2025-{i}")).unwrap();
    ...
    for i in 1..=3u64 {
        let r = db.assign(&req(&format!("doc-2026-{i}"), "sf-setubal", "oficio_at", "u1", None, None), dt(2026, 2, i as u32), &format!("ref-2026-{i}")).unwrap();
    ...
    let count_2025 = db.count_assignments(&AssignmentFilter {
        period_key: Some("2025".into()),
        ..Default::default()
    }).unwrap();
    assert_eq!(count_2025, 5);

    let count_2026 = db.count_assignments(&AssignmentFilter {
...
fn reset_mensal_do_contador() {
    ...
    // Janeiro: 3 certidões
    let r = db.assign(&req("c-jan-1", "sf-setubal", "certidao", "u1", None, None), dt(2026, 1, 5), "rj1").unwrap();
    assert_eq!(r.period_key, "2026-01");
    assert_eq!(r.number_value, "CERT/2026-01/0001");

    db.assign(&req("c-jan-2", "sf-setubal", "certidao", "u1", None, None), dt(2026, 1, 15), "rj2").unwrap();
    let r3 = db.assign(&req("c-jan-3", "sf-setubal", "certidao", "u1", None, None), dt(2026, 1, 28), "rj3").unwrap();
    assert_eq!(r3.sequence_value, 3);
    assert_eq!(r3.number_value, "CERT/2026-01/0003");

    // Fevereiro: contador reinicia
    let r_feb = db.assign(&req("c-fev-1", "sf-setubal", "certidao", "u1", None, None), dt(2026, 2, 3), "rf1").unwrap();
    ...
    let r_mar = db.assign(&req("c-mar-1", "sf-setubal", "certidao", "u1", None, None), dt(2026, 3, 10), "rm1").unwrap();
    assert_eq!(r_mar.period_key, "2026-03");
    assert_eq!(r_mar.sequence_value, 1);

    // Contagem por mês
    let jan = db.count_assignments(&AssignmentFilter { period_key: Some("2026-01".into()), ..Default::default() }).unwrap();
    let fev = db.count_assignments(&AssignmentFilter { period_key: Some("2026-02".into()), ..Default::default() }).unwrap();
    assert_eq!(jan, 3);
    assert_eq!(fev, 1);
}
...
fn validade_temporal_da_sequencia() {
    ...
    let active_2025 = db.find_active_for(
        &NumberingKind::Document, "sf-setubal", Some("oficio_at"), None,
        NaiveDate::from_ymd_opt(2025, 6, 1).unwrap(),
    ).unwrap().expect("deve encontrar sequência para 2025");
    assert_eq!(active_2025.sequence_id, "sf-setubal-oficio");
    assert_eq!(active_2025.padding, 5);

    let active_2026 = db.find_active_for(
        &NumberingKind::Document, "sf-setubal", Some("oficio_at"), None,
        NaiveDate::from_ymd_opt(2026, 3, 1).unwrap(),
    ).unwrap().expect("deve encontrar sequência para 2026");
    ...
    let mut db = open(&db_path);
    let r_2025 = db.assign(&req("old-doc-1", "sf-setubal", "oficio_at", "u1", None, None), dt(2025, 11, 10), "rold").unwrap();
    let r_2026 = db.assign(&req("new-doc-1", "sf-setubal", "oficio_at", "u1", None, None), dt(2026, 2, 5), "rnew").unwrap();
...
fn anulacao_de_numero() {
    ...
    let r1 = db.assign(&req("doc-anular", "sf-setubal", "oficio_at", "ccosta", Some("Erro de digitação"), None), now, "ref-anular").unwrap();
    assert_eq!(r1.number_value, "OF/2026/00001");
    assert!(matches!(r1.status, domain_numerador::AssignedStatus::Assigned));

    // Anular o ofício
    let void_req = ChangeStatusRequest {
        kind: NumberingKind::Document,
        target: TargetRef { id: "doc-anular".into(), target_type: "document".into() },
        actor: ActorRef { id: "chefe".into(), name: Some("Chefe de Serviço".into()) },
        reason: "Ofício emitido por engano".into(),
        correlation_id: None,
    };
    let voided = db.change_status(&void_req, AssignedStatus::Void, dt(2026, 4, 11)).unwrap();
    assert!(matches!(voided.status, AssignedStatus::Void));

    // Verificar que a anulação persiste após reabrir a DB
    let db = open(&db_path);
    let loaded = db.get_by_target(&NumberingKind::Document, "doc-anular").unwrap().unwrap();
    ...
    let mut db = open(&db_path);
    let err = db.change_status(&void_req, AssignedStatus::Void, dt(2026, 4, 12)).unwrap_err();
    assert!(matches!(err, NumeradorDomainError::InvalidStatusTransition(_)));

    // Emitir novo ofício após anulação — contador continua (números anulados não são reutilizados)
    let r2 = db.assign(&req("doc-seguinte", "sf-setubal", "oficio_at", "ccosta", None, None), dt(2026, 4, 11), "ref-seg").unwrap();
...
fn dashboard_contagens_e_filtros() {
    ...
    db.assign(&req("of-01", "sf-setubal", "oficio_at", "u1", Some("Janeiro 1"), None), jan, "ro1").unwrap();
    db.assign(&req("of-02", "sf-setubal", "oficio_at", "u1", Some("Janeiro 2"), None), jan, "ro2").unwrap();
    db.assign(&req("of-03", "sf-setubal", "oficio_at", "u2", Some("Março 1"), None), mar, "ro3").unwrap();
    db.assign(&req("of-04", "sf-setubal", "oficio_at", "u2", Some("Junho 1"), None), jun, "ro4").unwrap();
    db.assign(&req("of-05", "sf-setubal", "oficio_at", "u1", Some("Outubro 1"), None), out, "ro5").unwrap();
    db.assign(&req("of-06", "sf-setubal", "oficio_at", "u1", Some("Outubro 2"), None), out, "ro6").unwrap();

    // Despachos
    db.assign(&req("desp-01", "sf-setubal", "despacho", "chefe", None, None), jan, "rd1").unwrap();
    db.assign(&req("desp-02", "sf-setubal", "despacho", "chefe", None, None), mar, "rd2").unwrap();
    db.assign(&req("desp-03", "sf-setubal", "despacho", "chefe", None, None), out, "rd3").unwrap();

    // Dashboard: totais por tipo (sequence_id)
    let total_oficios = db.count_assignments(&AssignmentFilter {
        sequence_id: Some("sf-setubal-oficio".into()),
        ..Default::default()
    }).unwrap();
    assert_eq!(total_oficios, 6);

    let total_despachos = db.count_assignments(&AssignmentFilter {
    ...
    let outubro = db.count_assignments(&AssignmentFilter {
    ...
    // Recentes: últimos 3
    let recentes = db.list_assignments(&AssignmentFilter::default(), 3).unwrap();
    assert_eq!(recentes.len(), 3);
    // Mais recentes primeiro — os 3 de Outubro
    assert!(recentes.iter().all(|a| a.period_key == "2026"));

    // Por actor
    let by_u1 = db.list_assignments(&AssignmentFilter::default(), 100)
        .unwrap()
        .into_iter()
...
fn simulacao_municipio_dois_anos() {
    ...
    db.upsert(&oficio_seq("sf-setubal", from_2024, None)).unwrap();
    db.upsert(&despacho_seq("sf-setubal", from_2024)).unwrap();

    // SF Sintra: apenas ofícios (começa em 2026)
    db.upsert(&oficio_seq("sf-sintra", from_2026, None)).unwrap();
    ...
    // Q1 2025
    let of_2025_01 = db.assign(&req("of-2025-001", "sf-setubal", "oficio_at", "ccosta", Some("Pedido de esclarecimento"), Some("ACSS")), dt(2025, 1, 8), "rof-2025-001").unwrap();
    assert_eq!(of_2025_01.number_value, "OF/2025/00001");

    let of_2025_02 = db.assign(&req("of-2025-002", "sf-setubal", "oficio_at", "mmatos", Some("Resposta a ofício"), Some("DGO")), dt(2025, 2, 14), "rof-2025-002").unwrap();
    assert_eq!(of_2025_02.number_value, "OF/2025/00002");

    let dp_2025_01 = db.assign(&req("dp-2025-001", "sf-setubal", "despacho", "chefe", None, None), dt(2025, 2, 20), "rdp-2025-001").unwrap();
    assert_eq!(dp_2025_01.number_value, "DESP/2025/0001");

    // Certidões Jan/Fev 2025
    let c_jan1 = db.assign(&req("cert-2025-jan-1", "sf-setubal", "certidao", "balcao", Some("Certidão de residência"), None), dt(2025, 1, 10), "rc-jan1").unwrap();
    assert_eq!(c_jan1.number_value, "CERT/2025-01/0001");

    let c_jan2 = db.assign(&req("cert-2025-jan-2", "sf-setubal", "certidao", "balcao", Some("Certidão predial"), None), dt(2025, 1, 22), "rc-jan2").unwrap();
    assert_eq!(c_jan2.number_value, "CERT/2025-01/0002");

    let c_fev1 = db.assign(&req("cert-2025-fev-1", "sf-setubal", "certidao", "balcao", None, None), dt(2025, 2, 5), "rc-fev1").unwrap();
    assert_eq!(c_fev1.number_value, "CERT/2025-02/0001"); // reset mensal

    // Q4 2025 — último ofício do ano
    let of_2025_10 = db.assign(&req("of-2025-010", "sf-setubal", "oficio_at", "ccosta", Some("Relatório anual"), Some("Câmara Municipal")), dt(2025, 12, 18), "rof-2025-010").unwrap();
    ...
    let of_2026_01 = db.assign(&req("of-2026-001", "sf-setubal", "oficio_at", "mmatos", Some("Abertura de ano"), Some("DGAL")), dt(2026, 1, 5), "rof-2026-001").unwrap();
    assert_eq!(of_2026_01.number_value, "OF/2026/00001"); // reiniciou!
    assert_eq!(of_2026_01.period_key, "2026");

    let dp_2026_01 = db.assign(&req("dp-2026-001", "sf-setubal", "despacho", "chefe", None, None), dt(2026, 1, 10), "rdp-2026-001").unwrap();
    assert_eq!(dp_2026_01.number_value, "DESP/2026/0001"); // reiniciou!

    let of_2026_02 = db.assign(&req("of-2026-002", "sf-setubal", "oficio_at", "ccosta", None, Some("ATA")), dt(2026, 3, 12), "rof-2026-002").unwrap();
    assert_eq!(of_2026_02.number_value, "OF/2026/00002");

    // SF Sintra começa em 2026
    let of_sintra_01 = db.assign(&req("sintra-001", "sf-sintra", "oficio_at", "afernandes", Some("Pedido inicial"), None), dt(2026, 2, 1), "rsintra-001").unwrap();
    ...
    let of_set = db.get_by_target(&NumberingKind::Document, "of-2026-001").unwrap().unwrap();
    let of_sin = db.get_by_target(&NumberingKind::Document, "sintra-001").unwrap().unwrap();
    ...
    let void_req = ChangeStatusRequest {
        kind: NumberingKind::Document,
        target: TargetRef { id: "of-2026-002".into(), target_type: "document".into() },
        actor: ActorRef { id: "chefe".into(), name: None },
        reason: "Emitido para entidade errada".into(),
        correlation_id: None,
    };
    db.change_status(&void_req, AssignedStatus::Void, dt(2026, 3, 13)).unwrap();

    // Novo ofício depois da anulação — contador não recua
    let of_2026_03 = db.assign(&req("of-2026-003", "sf-setubal", "oficio_at", "ccosta", None, Some("ATA")), dt(2026, 3, 13), "rof-2026-003").unwrap();
    ...
    let total_of_setubal_2025 = db.count_assignments(&AssignmentFilter {
        sequence_id: Some("sf-setubal-oficio".into()),
        period_key: Some("2025".into()),
        ..Default::default()
    }).unwrap();
    ...
    let total_of_setubal_2026 = db.count_assignments(&AssignmentFilter {
        sequence_id: Some("sf-setubal-oficio".into()),
        period_key: Some("2026".into()),
        ..Default::default()
    }).unwrap();
    ...
    let activos_2026 = db.list_assignments(&AssignmentFilter {
        sequence_id: Some("sf-setubal-oficio".into()),
        period_key: Some("2026".into()),
    ...
    let all = db.list_assignments(&AssignmentFilter::default(), 1000).unwrap();
    let mut seen: std::collections::HashSet<(&str, &str, u64)> = std::collections::HashSet::new();
    for a in &all {
        let key = (a.sequence_id.as_str(), a.period_key.as_str(), a.sequence_value);
    ...
    assert_eq!(grand_total, all.len() as u64, "totais persistem após reabrir DB");
}
