...
use support_storage::{
    CryptoStorageProtector, JsonStorageCodec, ProtectedStorage, Storage, StorageKey,
    StorageNamespace,
};
...
#[test]
#[ignore = "stress test SQLite de 5 minutos; correr explicitamente com cargo test -p adapter-sqlite --test storage_sqlite_stress_tests -- --ignored --nocapture"]
fn sqlite_raw_storage_concurrent_reads_and_writes_for_five_minutes() {
    let dir = tempdir().unwrap();
    let db_path = dir.path().join("storage-stress.db");
    ...
    let namespace = StorageNamespace::new("stress.sqlite").unwrap();
    ...
    while started_at.elapsed() < STRESS_DURATION {
        ...
        eprintln!(
            "adapter-sqlite storage stress: elapsed={:?} reads={} writes={} errors={}",
            started_at.elapsed(),
    ...
    let metrics = storage.raw().metrics();
    storage.raw().shutdown().unwrap();

    eprintln!(
        "adapter-sqlite storage stress final: reads={final_reads} writes={final_writes} errors={final_errors} writer_metrics={metrics:?}"
    );
    ...
    assert_eq!(metrics.failed_commands, 0, "writer queue reported failures");
}

fn build_storage(
    config: SqliteConfig,
) -> ProtectedStorage<SqliteRawStorage, JsonStorageCodec, CryptoStorageProtector<StaticKeyProvider>>
{
    let raw = SqliteRawStorage::open(config).unwrap();
    let keys = StaticKeyProvider::new(SecretKey::new([41; support_crypto::KEY_LENGTH_BYTES]), None);

    ProtectedStorage::new(raw, JsonStorageCodec, CryptoStorageProtector::new(keys))
}
